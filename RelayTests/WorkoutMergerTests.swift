...
struct WorkoutMergerTests {
    ...
    func primarySourcePreserved() {
        let strava = workout(source: .strava, distance: 10000)
        let hk = workout(source: .healthKit, distance: 9900)
        let result = WorkoutMerger.merge([strava, hk], orderedSources: [.strava, .healthKit])
        #expect(result.source == .strava)
    }

    @Test("higher-priority source wins for non-nil fields")
    func higherPriorityWins() {
        let strava = workout(source: .strava, avgHR: 150)
    ...
    func nilFieldFallback() {
        let strava = workout(source: .strava, avgHR: nil)
        let hk = workout(source: .healthKit, avgHR: 145)
        let result = WorkoutMerger.merge([strava, hk], orderedSources: [.strava, .healthKit])
    ...
    func distanceFallback() {
        let primary = workout(source: .strava, distance: nil)
        let fallback = workout(source: .intervals, distance: 15000)
        let result = WorkoutMerger.merge([primary, fallback], orderedSources: [.strava, .intervals])
        #expect(result.distance == 15000)
    }

    @Test("calories falls back correctly")
    func caloriesFallback() {
        let primary = workout(source: .strava, calories: nil)
        let fallback = workout(source: .healthKit, calories: 500)
        let result = WorkoutMerger.merge([primary, fallback], orderedSources: [.strava, .healthKit])
    ...
    func elevationFallback() {
        let primary = workout(source: .strava, elevation: nil)
    ...
    func singleWorkout() {
        let w = workout(source: .strava, distance: 42195, avgHR: 155, avgPower: 230)
        let result = WorkoutMerger.merge([w], orderedSources: [.strava])
    ...
    func unknownSourceOrdering() {
        let strava = workout(source: .strava, avgHR: 160)
        let hk = workout(source: .healthKit, avgHR: 155)
        // healthKit not in ordered list — goes last, strava still wins
        let result = WorkoutMerger.merge([strava, hk], orderedSources: [.strava, .intervals])
    ...
    func threeSourceFallback() {
        let a = workout(source: .strava, avgHR: 150, avgPower: nil, elevation: nil)
        ...
        let result = WorkoutMerger.merge([a, b, c], orderedSources: [.strava, .intervals, .healthKit])
        #expect(result.avgHeartRate == 150)
